...
// Clase de utilidad para leer configuraciones desde archivo config.properties
public class ConfigReader { // declara la clase pública ConfigReader
    ...
    static { // bloque estático para que se ejecute una sola vez al cargar la clase
        try { // inicia bloque de manejo de excepciones
            ...
            fileInputStream.close(); // cierra el archivo
        } catch (IOException e) { // captura cualquier error de la lectura del archivo
            System.err.println("Error al cargar config.properties: " + e.getMessage()); // imprime el error en consola
                    e.printStackTrace(); // imprime la pila de excepciones para poder debuggar
        }
    }
